...
st.markdown(
    ...
    .block-container { max-width: 560px !important; padding-top: 2rem; padding-bottom: 2.5rem; }
    .hero-header {
      background: var(--accent); color: white; padding: 20px 24px 16px;
      display: flex; align-items: center; gap: 14px;
      border-radius: 0 0 14px 14px; box-shadow: 0 2px 8px rgba(0,0,0,.18); margin-bottom: 18px;
    }
    .cross { width: 38px; height: 38px; background: white; border-radius: 6px; display: grid; place-items: center; flex-shrink: 0; font-size: 22px; }
    .hero-title { font-size: 18px; font-weight: 700; line-height: 1.2; margin: 0; }
    .hero-subtitle { font-size: 12px; opacity: .78; margin-top: 2px; }
    .card { background: var(--card); border-radius: var(--radius); border: 1px solid var(--border); padding: 24px 20px; margin-bottom: 16px; box-shadow: 0 1px 4px rgba(0,0,0,.06); }
    .card-title { font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; color: var(--muted); margin-bottom: 18px; padding-bottom: 10px; border-bottom: 1px solid var(--border); }
    .legend { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 6px; }
    .legend-item { display: flex; align-items: center; gap: 5px; font-size: 11px; color: var(--muted); }
    .dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
    .ctas-badge { display: inline-flex; align-items: center; gap: 10px; border-radius: 8px; padding: 10px 18px; font-weight: 700; font-size: 22px; margin-bottom: 16px; color: white; }
    .ctas-badge small { font-size: 13px; font-weight: 600; display: block; opacity: .9; }
    .result-section { margin-top: 16px; }
    .result-section h4 { font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; color: var(--muted); margin-bottom: 8px; }
    .result-section p { font-size: 14px; line-height: 1.6; color: #000000; }
    .result-section p, .result-section strong, .result-section em, .result-section span { color: #000000 !important; }
    .red-flags { list-style: none; padding-left: 0; }
    .red-flags li { font-size: 14px; padding: 6px 10px; background: #FFF5F5; border-left: 3px solid var(--red-dark); color: var(--red-dark); border-radius: 0 6px 6px 0; margin-bottom: 6px; }
    .red-flags li::before { content: "🚩 "; color: var(--red-dark); }
    .helper-text { color: var(--muted); font-size: 0.92rem; }
    .pain-scale { display: flex; justify-content: space-between; padding: 0 4px; margin-bottom: 6px; }
    .pain-scale-item { display: flex; flex-direction: column; align-items: center; gap: 2px; font-size: 19px; line-height: 1; }
    .pain-scale-item .num { font-size: 10px; font-weight: 700; color: var(--muted); }
    .field-label { color: #000000 !important; font-size: 13px; font-weight: 600; margin-bottom: 6px; display: block; }
    .stTextInput label, .stNumberInput label, .stSelectbox label, .stTextArea label, .stRadio label, .stSlider label { color: #000000 !important; opacity: 1 !important; }
    div[data-testid="stForm"] label, div[data-testid="stForm"] p, div[data-testid="stForm"] span { color: #000000 !important; opacity: 1 !important; }
    div[data-baseweb="input"] input, div[data-baseweb="select"] > div, textarea, input { background: #ffffff !important; color: #000000 !important; -webkit-text-fill-color: #000000 !important; }
    div[data-baseweb="input"] input::placeholder, textarea::placeholder { color: #666666 !important; opacity: 1; }
    div[data-baseweb="select"] * { color: #000000 !important; }
    div[data-baseweb="radio"] label, div[data-baseweb="radio"] span { color: #000000 !important; opacity: 1 !important; font-weight: 600; }
    div[data-baseweb="radio"] *, div[data-baseweb="slider"] * { color: #000000 !important; }
    div[data-testid="stFormSubmitButton"] button { background: var(--red) !important; color: #ffffff !important; -webkit-text-fill-color: #ffffff !important; border: 1px solid var(--red) !important; font-weight: 800 !important; border-radius: 10px !important; padding: 0.9rem 1rem !important; }
    div[data-testid="stFormSubmitButton"] button:hover { background: #A93226 !important; }
    div[data-testid="stFormSubmitButton"] button * { color: #ffffff !important; -webkit-text-fill-color: #ffffff !important; }
    </style>
    """,
    unsafe_allow_html=True,
)
...
with st.container():
    ...
    if st.session_state["submitted_result"] is None:
        with st.form("patient_form"):
            ...
            symptom = st.text_area("Chief Complaint", placeholder="Describe the main symptom or reason for visit…", label_visibility="collapsed")
            field_label("Duration")
            duration = st.text_input("Duration", placeholder="e.g. 2 hours, since yesterday morning", label_visibility="collapsed")
            field_label("Medical History")
            history = st.text_area("Medical History", placeholder="e.g. hypertension, diabetes, allergies…", height=70, label_visibility="collapsed")
        ...
        if submitted:
            if gender == "Select…":
                ...
                triage_score, triage_label, reasons = assess_triage(
                    age=age, pain=pain, fever=37.0, symptom_text=symptom,
                    duration_hours=duration_hours, history=history, er_location=er_location,
                )
                wait_minutes = estimate_wait_time(triage_score, 0)
                result_data = {
                    "name": name, "age": age, "gender": gender, "er_location": er_location,
                    "triage_score": triage_score, "triage_label": triage_label,
                    "reasons": reasons, "wait_minutes": wait_minutes,
                    "symptom": symptom, "history": history,
                }
                st.session_state["submitted_result"] = result_data
                append_pending({
                    "id": str(uuid.uuid4()),
                    "submitted_at": datetime.now().isoformat(),
                    "name": name, "age": int(age), "sex": gender, "er": er_location,
                    "ctas": triage_score, "ctas_label": triage_label,
                    "complaint": symptom, "history": history,
                    "reasons": reasons, "wait_minutes": wait_minutes,
                })
                st.rerun()
        else:
            st.markdown('<p class="helper-text">Fill out the form and submit to generate a CTAS-style result card, red flags, and a queue estimate.</p>', unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            <div class="ctas-badge" style="background:{badge_colors[result['triage_score']]}">
              ...
              <div>CTAS {result['triage_score']}<small>{result['triage_label']}</small></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown('<div class="result-section"><h4>Red Flags</h4></div>', unsafe_allow_html=True)
        ...
        st.markdown("<ul class='red-flags'>" + "".join(f"<li>{flag}</li>" for flag in red_flags) + "</ul>", unsafe_allow_html=True)
